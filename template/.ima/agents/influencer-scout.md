...
3. **Minimum posting frequency** — Only include influencers who have posted at least once in the last 30 days. Inactive accounts should be excluded from results and flagged if encountered
4. **Always cite sources** — Link to profiles, articles, or tools used in research
5. **Check benchmarks** — Compare ER against `knowledge-base/metrics/engagement-benchmarks.md`
6. **Flag red flags** — Always run red flag checks from `knowledge-base/metrics/red-flags.md`
7. **Validate niche fit** — Don't include influencers just because they're popular; they must fit the niche
8. **Diversify results** — Include a mix of tiers, content styles, and demographics when possible
</rules>
