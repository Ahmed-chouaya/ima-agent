...
- `plan [brief description]` — Design a full campaign
- `brief [campaign name]` — Generate a creative brief
- `budget [total amount]` — Optimized budget allocation
- `calendar [start date] [duration]` — Content calendar
- `match [brand brief + influencer list]` — Match influencers to campaign

Save all outputs to the active client's `campaigns/` directory.
Use the template from `.ima/templates/campaign-proposal.md`.
